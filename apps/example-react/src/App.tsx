...
function Shell({ mode, setMode }: { mode: Mode; setMode: (m: Mode) => void }) {
  ...
  if (route.kind === "security") return <SecurityPage mode={mode} setMode={setMode} />;
  if (route.kind === "docs") return <Docs current={route.page} mode={mode} setMode={setMode} />;
...
function parseHashRoute(hash: string): Route {
  ...
  if (stripped === "security") return { kind: "security" };
...
function SecurityPage({ mode, setMode }: { mode: Mode; setMode: (m: Mode) => void }) {
  const toggle = () => setMode(mode === "dark" ? "light" : "dark");
  const html = useMemo(() => renderMarkdown(securityMd), []);
  useEffect(() => { window.scrollTo(0, 0); }, []);

  return (
    <div className="landing security-shell" data-theme={mode}>
      <header className="landing-topbar">
        ...
        <nav className="landing-nav">
          ...
          <a href="#/docs/introduction">Docs</a>
          <a href="#/security" data-active="true">Security</a>
          <button
            type="button"
            className="landing-theme-toggle"
            onClick={toggle}
      ...
      <main className="security-content">
        ...
        <div className="security-footer">
          <a className="landing-btn-ghost" href="#/docs/introduction">
            Read the full docs
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
              strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
              <line x1="5" y1="12" x2="19" y2="12" />
