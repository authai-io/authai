...
describe("issueSessionJwt / verifySessionJwt", () => {
  it("round-trips recordId, recordKey, and provider", async () => {
    const recordKey = generateRecordKey();
    const recordId = "01HNYZTESTID";
    for (const provider of ["openai", "xai", "github"] as const) {
      const jwt = await issueSessionJwt({ recordId, recordKey, provider, secret });
  ...
  it("accepts legacy v1 JWTs (no prov field) and defaults provider to openai", async () => {
    const recordKey = generateRecordKey();
    ...
    const legacy = await new SignJWT({
      ...
      .sign(secret);
    const verified = await verifySessionJwt(legacy, secret);
    expect(verified.provider).toBe("openai");
  });

  it("fails with the wrong secret", async () => {
    const recordKey = generateRecordKey();
    const jwt = await issueSessionJwt({ recordId: "id", recordKey, provider: "openai", secret });
  ...
  it("issued JWT has 14-day expiry", async () => {
    ...
    expect(payload.prov).toBe("openai");
  });
});
